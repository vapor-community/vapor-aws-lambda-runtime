...
extension EventLoopFuture where Value == TodoItem {
    func fixTodoItemBaseUrl(with _: Vapor.Request) -> EventLoopFuture<Value> {
        return map { (todo) -> (TodoItem) in
            var newTodo = todo
...
todos.delete { _ -> EventLoopFuture<[TodoItem]> in
